...
app.controller('POIListController', function ($scope, $http) {
  ...
  var proxyURL = "http://" + window.location.hostname + ":8000";

  // TODO Change recommendation service address to your own
  var recommendationServiceAddress = "vertx2go.recommendations";
  var recommendationBroadcastAddress = "vertx2go.recommendations.announce";
  ...
  eb.onopen = function () {
    ...
    $http.get(window.location.protocol + '//' + window.location.host + '/places').success(function (data) {
